...
void largest_and_second_largest(int a[], int n, int *m, int *s)
{
	int *I,                       //stores the losers
		...
		i, j,                     //indexed
		lu,                       //stores the last compared element in the current level
	...
	for (i=0;i<n;i++)             //initialize - no loser yet
		I[i]=-1;
	for (;;) 
	{                    //loop until size-1 becomes more than n
		i=size-1;
		if (i>=n)
			break;                    //loop exit
		j=size+i;
		...
		for (;;)
		{                  //another seemingly infinite loop
			...
			else 
			{
				...
				if (j>=n)
					...
					{
						lu=i;               //if there is no earlier last used, store the current number as last used
						break;
					}
			}
		}
